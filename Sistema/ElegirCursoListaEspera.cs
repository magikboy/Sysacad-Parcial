...
namespace Sistema
{
    public partial class ElegirCursoListaEspera : Form
    {
        Label[] labels;
        public ElegirCursoListaEspera()
        {
            InitializeComponent();
            labels = new Label[]
            {
        ...
        private void CargarYMostrarDatosDesdeBaseDeDatos()
        {
            try
            {
                ...
                using (MySqlConnection connection = new MySqlConnection(connectionString))
                {
                    ...
                    string selectQuery = "SELECT id, nombre FROM cursos WHERE CuposDisponibles = 0";
                ...
                using (MySqlConnection connection = new MySqlConnection(connectionString))
                {
                    ...
                    using (MySqlDataReader reader = command.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            string nombreCurso = reader["nombre"].ToString();
                            this.Hide();
                            //manejo lista de espera
                            ManejoListaEspera manejoListaEspera = new ManejoListaEspera(cursoId, nombreCurso);
                            manejoListaEspera.Show();
