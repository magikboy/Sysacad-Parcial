...
namespace Sistema
{
    public partial class ElegirCursoAcademico : Form
    {
        Label[] labels;
        public ElegirCursoAcademico()
        {
            InitializeComponent();
            labels = new Label[]
            {
                ...
                using (MySqlConnection connection = new MySqlConnection(connectionString))
                {
                    ...
                    using (MySqlDataReader reader = command.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            string nombreCurso = reader["nombre"].ToString();
                            this.Hide();
                            //editar requisitos academicos
                            EditarRequisitosAcademicos editarRequisitosAcademicos = new EditarRequisitosAcademicos(cursoId, nombreCurso);
                            editarRequisitosAcademicos.Show();
                        }
                        else
                        {
